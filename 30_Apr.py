...
2.what is the result of following expression  python ?
    ...
    a)True
    b)False
    c)Error  ------------------->This is Correct Answer.
...
1. Numerical Derivative
    Problem Description:
    The derivative of a function is a value that tells us how much the output of a mathematical function would change, if we were to make a very, very tiny change in its input.
    In mathmetical terms, the limit definition of a derivative is defined as: limo Where x and hare both inputs to the h
    ...
    Output:
    9.0
    Note: The value of h will always be more than for this problem.
...
5.Module Trouble
    Problem Description:
    Shikha is trying to solve a hard math problem in which she is required to take the mod of a number x with y quite frequently. 
    Given two numbers x and y write a program that helps Shikha do this easily. Assume that the value of y is always greater than 0.
    Input Format:
...
8.Are the weights balanced?
    Problem Description:
    A weighing machine has two arms, a left arm, and a right arm. On both sides of the weighting machine we can put in weights and if both of those weights are equal, the arms of the machine will hang equally in the air, with none of them hanging below the other. 
    This is hard to observe visually hence you are asked to write a program that takes in two weight values as input and outputs True if they will leave the machine balanced and False if they will leave the machine unbalanced.
    Input Format:
    ...
    Output:
    False
right=float(input("Enter Right Weighting:- "))
left=float(input("Enter Left Weighting:- "))
if(right==left):
    print("True")
else:
    print("False")
